...
# ALLOWED VIDEO TYPES
VIDEO_EXTENSIONS = FileExtensionValidator(allowed_extensions=["mp4", "mkv", "webm"])
...
class Topic(models.Model):
    ...
    recorded_video = models.FileField(
        upload_to="topics/", blank=True, null=True, validators=[VIDEO_EXTENSIONS]
    )
...
class ResourceExtra(models.Model):
    topic = models.ForeignKey(
        Topic, on_delete=models.CASCADE, related_name="extras"
    )  # references the above topic field
    ...
    file = models.FileField(
        ...
        blank=True,
        null=True,
        validators=[DOCUMENT_EXTENSIONS],
    )  # for books,notes,pdf's
    description = models.TextField(blank=True)
